...
class EnvironmentSetup(object):
    ...
    def __init__(self):
        ...
        self.logger = get_qs_logger(log_file_prefix="CloudShell Sandbox Setup", log_group=self.reservation_id, log_category='Setup')
    ...
    def execute(self):
        ...
        apps_in_reservation = [app.Name for app in reservation_details.ReservationDescription.Apps]
        deploy_result = self._deploy_apps_in_reservation(api=api, reservation_details=reservation_details)

        self._try_exeucte_autoload(api=api, deploy_result=deploy_result, resource_details_cache=resource_details_cache)

        # refresh reservation_details after app deployment if any deployed apps
        if deploy_result and deploy_result.ResultItems:
            reservation_details = api.GetReservationDetails(self.reservation_id)

        self._connect_all_routes_in_reservation(api=api, reservation_details=reservation_details, reservation_id=self.reservation_id, resource_details_cache=resource_details_cache)
        ...
        api.WriteMessageToReservationOutput(reservationId=self.reservation_id, message='Configuring CloudFront')
        ...
        for deployed_app in deploy_result.ResultItems:
            ...
            try:
                ...
                if not message_written:
                    api.WriteMessageToReservationOutput(reservationId=self.reservation_id, message='Apps are being discovered...')
                ...
                api.ExecuteCommand(self.reservation_id, deployed_app_name, TARGET_TYPE_RESOURCE, REMAP_CHILD_RESOURCES, [])

            except CloudShellAPIError as exc:
                if exc.code not in (EnvironmentSetup.NO_DRIVER_ERR,  EnvironmentSetup.DRIVER_FUNCTION_ERROR, MISSING_COMMAND_ERROR):
                    self.logger.error(
                        "Error executing Autoload command on deployed app {0}. Error: {1}".format(deployed_app_name, exc.rawxml))
                    api.WriteMessageToReservationOutput(reservationId=self.reservation_id, message='Discovery failed on "{0}": {1}'.format(deployed_app_name, exc.message))
    ...
    def _deploy_apps_in_reservation(self, api, reservation_details):
        ...
        if not apps or (len(apps) == 1 and not apps[0].Name):
            ...
            api.WriteMessageToReservationOutput(reservationId=self.reservation_id, message='No apps to deploy')
            return None

        app_names = map(lambda x: x.Name, apps)
        app_inputs = map(lambda x: DeployAppInput(x.Name, "Name", x.Name), apps)

        api.WriteMessageToReservationOutput(reservationId=self.reservation_id, message='Apps deployment started')
        self.logger.info("Deploying apps for reservation {0}. App names: {1}".format(reservation_details, ", ".join(app_names)))
    ...
    def _connect_all_routes_in_reservation(self, api, reservation_details, reservation_id, resource_details_cache):
        ...
        api.WriteMessageToReservationOutput(reservationId=self.reservation_id, message='Connecting all apps')
        res = api.ConnectRoutesInReservation(self.reservation_id, endpoints, 'bi')
        return res

    def _run_async_power_on_refresh_ip(self, api, reservation_details, deploy_results, resource_details_cache, reservation_id):
        """
        :param CloudShellAPISession api:
        :param GetReservationDescriptionResponseInfo reservation_details:
        ...
        if len(resources) == 0:
            api.WriteMessageToReservationOutput(reservationId=self.reservation_id, message='No resources to power on')
        ...
        async_results = [pool.apply_async(self._power_on_refresh_ip, (api, lock, message_status, resource, deploy_results, resource_details_cache))
