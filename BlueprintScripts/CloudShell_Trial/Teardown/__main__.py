...
api.WriteMessageToReservationOutput(reservationContext["id"], "1. Starting 'End Trial' Hubspot workflow")
...
api.WriteMessageToReservationOutput(reservationContext["id"], "2. Generating, sending and attaching analytics report")
...
api.ExecuteCommand(reservationContext["id"], "Admin-Google Analytics", "Service", "attach_ga_user_report_to_reservation", [InputNameValue(k,v) for k,v in command_inputs.items()], True)

# remove user from groups and deactivate
api.WriteMessageToReservationOutput(reservationContext["id"], "3. Deactivating user and removing permissions")
api.RemoveUsersFromGroup([username], group_name)
api.UpdateUser(username, email, isActive=False)

api.WriteMessageToReservationOutput(reservationContext["id"], "4. Archiving Trial Domain")
...
api.WriteMessageToReservationOutput(reservationContext["id"], "5. Sending 'Trial End' e-mails")
...
api.WriteMessageToReservationOutput(reservationContext["id"], "6. Verifying Domain cleanup")
